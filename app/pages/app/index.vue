...
import HealthBadge from '~/components/HealthCheck.vue';
...
<template>
  <main class="h-[calc(100vh-3rem)] grid grid-cols-1 md:grid-cols-5 xl:w-[1000px] mx-auto px-2 md:px-0">
    <div class="fixed top-16 right-0 mr-2 z-30 min-w-[180px]">
      <HealthBadge />
    </div>
    <div
      ...
      <MotionConfig :transition="{ type: 'tween', duration: 0.3, ease: 'anticipate' }">
        ...
        <motion.div v-if="user.data" :initial="{ filter: 'blur(10px)', scale: 0.8 }"
          :animate="{ filter: 'blur(0px)', scale: 1 }" :transition="{ delay: 0.2 }"
